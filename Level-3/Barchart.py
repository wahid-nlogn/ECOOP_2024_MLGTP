...
def load_model():
    ...
    if BENCHMARK=='Raytrace':
        with open('Level_2\models\modelraytrace.pkl', 'rb') as f:
            model = pickle.load(f)
    elif BENCHMARK=="Scimark":
        with open('Level_2\models\modelscimark.pkl', 'rb') as f:
            model = pickle.load(f)
    elif BENCHMARK=="Sieve":
        with open('Level_2\models\\modelsieve.pkl', 'rb') as f:
            model = pickle.load(f)
    elif BENCHMARK == "Nbody":
        with open('Level_2\models\modelnbody.pkl', 'rb') as f:
            model = pickle.load(f)
    elif BENCHMARK=='Pascal':
        with open('Level_2\models\modelpascal.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=='Meteor':
        with open('Level_2\models\modelmeteor.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=='Monte':
        with open('Level_2\models\modelmonte.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=='Chaos':
        with open('Level_2\models\modelchaos.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=='Richard':
        with open('Level_2\models\modelrichard.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=="Pdf":
        with open('Level_2\models\modelzebrapdf.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=="Benchfirst":
        with open('Level_2\models\modelbenchfirst.pkl','rb') as f:
            model=pickle.load(f)
    elif BENCHMARK=="Cpu":
        with open('Level_2\models\modelcpu.pkl','rb') as f:
            model=pickle.load(f)
    return model
def find_clusters(y):
...
def draw_plot():
    ...
    barWidth = .85
    plt.bar(r,zero_to_five, color='g', edgecolor='white', width=barWidth,tick_label="Wahid",label="0%-5%")
