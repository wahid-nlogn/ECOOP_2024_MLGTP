...
def cast(env, ctx, val, src, trg, msg, cast_function='retic_cast', misc=None):
    ...
    if not trg.top_free() or not subcompat(src, trg, env, ctx):
        ...
        return val
    if trg == Dyn and relations.prim(src): #This is a hotfix to prevent adding unnecessary calls to "cast" 
        return val
...
## STATEMENTS ##
    ...
    def visitFunctionDef(self, n, env, misc): #TODO: check defaults, handle varargs and kwargs
        ...
        #NOTE: changing the cast here so that higher order casts don't proliferate throughout the program

        #NOTE: removing the proxies from the program by casting the function from nty to nty instead of Dyn to nty
    ...
    def visitReturn(self, n, env, misc):
        if n.value:
            ...
            if (misc.ret != Dyn): #Only do the casting when the return type isn't Dyn
    ...
    def visitAssign(self, n, env, misc):
        ...
        if targets:
            ...
            if len(targets) == 1:
                ...
                #NOTE: removing the cast on assignments for now
    ...
    def visitFor(self, n, env, misc):
        ...
        elif tyinstance(ity, Iterable):
            iter_ty = Iterable(tty)
        else: iter_ty = Dyn
...
### EXPRESSIONS ###
    ...
    def visitBinOp(self, n, env, misc):
        ...
        #NOTE: removing this check which throws static type errors for now
        #if not ty.top_free():
