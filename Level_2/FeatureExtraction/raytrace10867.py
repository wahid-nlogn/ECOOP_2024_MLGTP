...
def add(vec:Tuple(Dyn,Dyn,Dyn), other)-> Tuple(Dyn,Dyn,Dyn):
    (x,y,z) = vec
    (x1,y1,z1) = other
...
def scale(vec, factor)->Tuple(Dyn,Dyn,Dyn):
    x,y,z = vec
    return (factor * x, factor * y, factor * z)

def cross(vec, other:Tuple(Dyn,Dyn,Dyn))->Tuple(Dyn,Dyn,Dyn):
    #other.mustBeVector()
    (x,y,z) = vec
    (x1,y1,z1) = other
    return (y * z1 - z * y1,
...
def reflectThrough(vec, normal)->Tuple(Dyn,Dyn,Dyn):
...
def halfspace(point, normal)->Tuple(Dyn, Tuple(Dyn,Dyn,Dyn)):
    #self.point = point
