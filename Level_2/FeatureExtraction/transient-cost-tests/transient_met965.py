...
(WIDTH, HEIGHT) = (5, 10)
DIR_NO = 6
(S, E) = check_type_tuple(((WIDTH * HEIGHT), 2), 2)
SE = (S + (E / 2))
SW = (SE - E)
...
def rotate(ido):
    ...
    return check_type_list([rd[o] for o in ido])
...
def flip(ido):
    ...
    return check_type_list([fd[o] for o in ido])

def permute(ido, r_ido):
    check_type_list(ido)
    check_type_list(r_ido)
    ps = []
    check_type_void(check_type_function(ps.append)(ido))
    for r in check_type_function(xrange)((DIR_NO - 1)):
        check_type_void(check_type_function(ps.append)(check_type_list(rotate(check_type_list(ps[(- 1)])))))
        for i in check_type_function(range)(check_type_function(len)(ps)):
            pp = ps[check_type_int(i)]
            check_type_void(check_type_function(ps.append)(check_type_list(flip(check_type_list(pp)))))
    return ps

def convert(ido):
    out = [0.0]
    for o in ido:
        check_type_void(check_type_function(out.append)(check_type_float((check_type_float(out[(- 1)]) + o))))
...
def get_footprints(board, cti, pieces):
    ...
    for c in board:
        for (pi, p) in check_type_function(enumerate)(pieces):
            ...
            for pp in p:
                fp = check_type_function(frozenset)(check_type_list([cti[(c + o)] for o in pp if ((c + o) in cti)]))
...
def get_senh(board, cti):
    ...
    nh = [E, SW, SE]
    for c in board:
        check_type_void(check_type_function(se_nh.append)(check_type_function(frozenset)(check_type_list([cti[(c + o)] for o in nh if ((c + o) in cti)]))))
    return se_nh

def get_puzzle(width, height):
    board = check_type_list([(((E * x) + (S * y)) + (y % 2)) for y in check_type_function(xrange)((height + 0)) for x in check_type_function(xrange)((width + 0))])
    ...
    p = check_type_function(list)(perms)
    pieces = [check_type_list([check_type_list(convert(pp)) for pp in p[0]])]
            ...
            if (fp <= free):
                ...
                if (check_type_function(len)(pieces_left) > 1):
                    ...
                    if (check_type_function(len)((n_free & se_nh[n_i_min])) > 0):
                        ...
                        check_type_function(check0(n_pieces_left).remove)(p)
                    ...
                    check_type_function(check1(solutions).insert)(check_type_function(bisect)(solutions, s), s)
                    rs = s[::(- 1)]
                    check_type_function(check1(solutions).insert)(check_type_function(bisect)(solutions, rs), rs)
...
def bench_meteor_contest(loops, board, pieces, solve_arg, fps, se_nh):
    range_it = check_type_function(xrange)(loops)
    for _ in range_it:
...
def main():
    (board, cti, pieces) = check_type_tuple(get_puzzle(WIDTH, HEIGHT), 3)
    fps = check_type_list(get_footprints(board, cti, pieces))
    se_nh = check_type_list(get_senh(check_type_list(board), cti))
    solve_arg = SOLVE_ARG
    bench_meteor_contest(4, board, pieces, solve_arg, fps, se_nh)
t0 = check_type_function(check2(time).time)()
main()
t1 = check_type_function(check2(time).time)()
