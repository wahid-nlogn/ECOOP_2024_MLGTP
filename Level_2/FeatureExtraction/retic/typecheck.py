...
if __name__ == '__main__' or __name__ == 'typecheck':
    ...
    import typing, utils, flags, rtypes, reflection, annotation_removal, runtime, ast_trans
    from runtime import tyinstance as retic_tyinstance
    from guarded import retic_dynfunc
    from settings import functionCosts, callGraph, definedFuncs, functionCostsNli, nli, callGraphNli, parCastedTimesAll,valuesCastedWithinFunc, contains_classes
    import settings
else:
    from .vis import Visitor
    from .gatherers import FallOffVisitor, WILL_RETURN
    ...
    from .errors import errmsg, static_val
    from .astor.misc import get_binop
    from . import typing, utils, flags, rtypes, reflection, annotation_removal, runtime, ast_trans
    from .runtime import tyinstance as retic_tyinstance
    from .guarded import retic_dynfunc
    from .settings import functionCosts, callGraph, definedFuncs, functionCostsNli, nli, callGraphNli,valuesCastedWithinFunc, contains_classes
    from . import settings


def print_ancestor_class_names(node, indent=0):
    print (' ' * indent, node.__class__.__name__)
    if hasattr(node,'parent'):
        print_ancestor_class_names(node.parent, indent+1)
        ast.dump(node.parent)

def is_child_of_assignment(node):
...
loopNameStack = ['once','once']

class PerFunc:
    def __init__(self,name,parIdxMap,castedVariables,parAssigned):
        self.name = name
        self.parIdxMap = parIdxMap
        self.castedVariables = castedVariables
        self.parAssigned = parAssigned

funcPVInfoStack = [PerFunc('Top',{},set(),set())]

def getLocalNli():
    # return settings.nli[loopNameStack[-1]]
    # print(loopNameStack[-1])
    return settings.nli.get(loopNameStack[-1],0)

def getGlobalNli():
    return settings.nli.get(loopNameStack[-2],0)

def find_caller(node):
    if isinstance(node,ast.FunctionDef):
        return node.name
    elif isinstance(node,ast.Module):
        return "TopLevel"
    elif hasattr(node,'parent'):
        return find_caller(node.parent)
    else:
        return "NoCaller"
...
def total_cost(src,trg):
    return create_cost(src,trg) + call_cost(src,trg)

def dict_access_cost(src,trg):
    if retic_tyinstance(trg, rtypes.Dyn):
        readrt = Function(AnonymousParameters([Dyn]), Dyn)
        writert = Function([Dyn, Dyn], Void)
    else:
        readrt = Function(AnonymousParameters([trg.keys]), trg.values)
        writert = Function([trg.keys, trg.values], Void)

    if retic_tyinstance(src, rtypes.Dyn):
        readsrc = Function(AnonymousParameters([Dyn]), Dyn)
        writesrc = Function([Dyn, Dyn], Void)
    else:
        readsrc = Function(AnonymousParameters([src.keys]), src.values)
        writesrc = Function([src.keys, src.values], Void)

    readcost = 25.96 * UNIT_OF_COST + call_cost(readsrc, readrt)
    writecost = 27.37 * UNIT_OF_COST + call_cost(writesrc,writert)

    if retic_tyinstance(src, rtypes.Dyn) or retic_tyinstance(trg, rtypes.Dyn):
        return (readcost + writecost)/2
    else:
        return (readcost + writecost)/2 + 3 * UNIT_OF_COST


def list_access_cost(src,trg):
    if retic_tyinstance(trg, rtypes.Dyn):
        readrt = Function(AnonymousParameters([Int]), Dyn)
        writert = Function(AnonymousParameters([Int,Dyn]), Dyn)
    else:
        readrt = Function(AnonymousParameters([Int]), trg.type)
        writert = Function(AnonymousParameters([Int,trg.type]), Dyn)

    if retic_tyinstance(src, rtypes.Dyn):
        readsrc = Function(AnonymousParameters([Int]), Dyn)
        writesrc = Function(AnonymousParameters([Int,Dyn]), Dyn)
    else:
        readsrc = Function(AnonymousParameters([Int]), src.type)
        writesrc = Function(AnonymousParameters([Int,src.type]),Dyn)

    readcost = 26.33 * UNIT_OF_COST + call_cost(readsrc, readrt)
    writecost = 27.67 * UNIT_OF_COST + call_cost(writesrc,writert)
...
functionCastCost = 0
#functionCastCostNliGlobal = 0
functionCastCostNliLocal = 0

# Approximate the cost that the casts are invoked. For a list cast, this cost
# is the average of the read and write access. For a tuple cast, this is the
# cost of reading tuple elements. For functions, this is the cost of the function call.
def call_cost(src,trg):
    if src == trg:
        return 0
    if retic_tyinstance(trg, rtypes.Dyn):
        if retic_tyinstance(src, rtypes.Function):
            return call_cost(src,retic_dynfunc(trg))

        elif retic_tyinstance(src, rtypes.Object) or retic_tyinstance(src, rtypes.Class):
            if all( retic_tyinstance(src.members[m],rtypes.Dyn) for m in src.members):
                return 0.004 * UNIT_OF_COST
            else: # Object () => Dyn has no call cast because it is unclear which
                  # object field will be accessed and/or invoked
                  # newly implemented for object costs
                return 0
                print ("1*Call cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
                with open("unimplemented.txt","a+") as err:
                    err.write ("Cost estimation not implemented for casts {} => {}".format(src,trg))
                return 1

        elif retic_tyinstance(src, typing.List):
            return list_access_cost(src,trg)

        elif retic_tyinstance(src,typing.Tuple):
            return 5.8 * UNIT_OF_COST

        elif retic_tyinstance(src, typing.Dict):
            return dict_access_cost(src,trg)

        elif any(retic_tyinstance(src, collection) for collection in [typing.Set]):
            print ("2*Call cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 1
        else: return 0
    elif retic_tyinstance(src, rtypes.Dyn):
        if retic_tyinstance(trg, rtypes.Function):
            return call_cost(retic_dynfunc(src),trg)

        # For objects, the call cost is attribute access
        elif retic_tyinstance(trg, rtypes.Object) or retic_tyinstance(trg, rtypes.Class):
            if all( retic_tyinstance(trg.members[m],rtypes.Dyn) for m in trg.members):
                return 0.004 * UNIT_OF_COST
            elif len(trg.members) > 1:
                # more than one element in the target. The call cost is 0 because
                # it is unclear which field will be accessed.
                return 0
            else:
                # there is only one member in target. The goal of this cast is to access
                # the field. Therefore, we add the cost of accessing it.
                ftype = next(iter(trg.members.values()))
                access_cost = total_cost(rtypes.Dyn,ftype)
                if retic_tyinstance(ftype,rtypes.Dyn):
                    access_cost += 0.3*UNIT_OF_COST
                elif retic_tyinstance(ftype,rtypes.Int):
                    access_cost += 0.75*UNIT_OF_COST
                elif retic_tyinstance(ftype,rtypes.Function):
                    access_cost += 15*UNIT_OF_COST
                else:
                    access_cost += 1.1 * UNIT_OF_COST
                return access_cost
                print ("3*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
                with open("unimplemented.txt","a+") as err:
                    err.write ("Cost estimation not implemented for casts {} => {}".format(src,trg))
                return 1

        elif retic_tyinstance(trg,typing.Tuple):
            return 5.9 * UNIT_OF_COST

        elif retic_tyinstance(trg,typing.List):
            return list_access_cost (src,trg)

        elif retic_tyinstance(trg,typing.Dict):
            return dict_access_cost (src,trg)

        elif any(retic_tyinstance(trg, collection) for collection in [typing.Set]):
            print ("A*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            with open("unimplemented.txt","a+") as err:
                err.write ("A*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))

            return 15.3*UNIT_OF_COST
        else:
            return 0
        ...
        length = max(src_frms.len(),trg_frms.len())

        cost = 0.48 * UNIT_OF_COST
        for s,t in zip( src_frms.types(length), trg_frms.types(length) ):
            if not retic_tyinstance(s, rtypes.Function) and not retic_tyinstance(t, rtypes.Function):
                cost += create_cost(t,s)
            else:
                cost += total_cost(t,s)

        if not retic_tyinstance(src_to, rtypes.Function) and not retic_tyinstance(trg_to, rtypes.Function):
            cost += create_cost(src_to,trg_to)
        else:
            cost += total_cost(src_to,trg_to)

        return cost

    elif retic_tyinstance(src, typing.Object) and retic_tyinstance(trg, typing.Object):
        if len(trg.members) > 1:
            # more than one element in the target. The call cost is 0 because
            # it is unclear which field will be accessed.
            return 0
        else:
            # there is only one member in target. The goal of this cast is to access
            # the field. Therefore, we add the cost of accessing it.
            # new implementation for object casts
            tkey = next(iter(trg.members.keys()))
            ttype = trg.members[tkey]
            ftype = src.members.get(tkey,rtypes.Dyn)
            access_cost = total_cost(ftype, ttype)
            if retic_tyinstance(ttype, rtypes.Dyn):
                access_cost += 0.3 * UNIT_OF_COST
            elif retic_tyinstance(ttype, rtypes.Int):
                access_cost += 0.75 * UNIT_OF_COST
            elif retic_tyinstance(ttype, rtypes.Function):
                access_cost += 15 * UNIT_OF_COST
            else:
                access_cost += 1.1 * UNIT_OF_COST
            return access_cost
        ...
        if retic_tyinstance(trg, typing.Class):
            print ("C*Call cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 25.3*UNIT_OF_COST
        elif retic_tyinstance(trg, typing.Function):
            print ("D*Call cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 25.3*UNIT_OF_COST
        else: raise ReticUnimplementedException(src, trg)
    elif retic_tyinstance(src, typing.Tuple) and retic_tyinstance(trg, typing.Tuple) :
        return 5.9 * UNIT_OF_COST

    elif retic_tyinstance(src, typing.List) and retic_tyinstance(trg, typing.List) :
        return list_access_cost(src,trg)

    elif retic_tyinstance(src, typing.Dict) and retic_tyinstance(trg, typing.Dict) :
        return dict_access_cost(src,trg)

    elif any(retic_tyinstance(src, collection) and retic_tyinstance(trg, collection) \
                 for collection in [typing.Set]):
        #retic_assert(retic_has_type(val, trg), '%s at line %s' % (msg, line))
        print ("E*Call cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
        return 32.3*UNIT_OF_COST
    elif retic_subcompat(src, trg):
        print ("F*Call Cost estimation not implemented for casts \n{} => {}\n".format(src,trg))
        return 32.3*UNIT_OF_COST
    else:
        raise ReticUnimplementedException(src, trg)

def objToDynCreate(src,trg):
    objcost = 0
    for m in src.members:
        if retic_tyinstance(src.members[m], rtypes.Dyn):
            objcost += 1.45
        elif retic_tyinstance(src.members[m], rtypes.Function):
            objcost += 2.2
        elif retic_tyinstance(src.members[m], rtypes.Object):
            objcost += 1.4 * len(src.members[m].members)
        else:  # Otherwise the type is Int. May need to refine
            objcost += 1.4
    return (8.5 + objcost) * UNIT_OF_COST

def dynToObjCreate(src,trg):
    objcost = 0
    for m in trg.members:
        if retic_tyinstance(trg.members[m], rtypes.Dyn):
            objcost += 1.25
        elif retic_tyinstance(trg.members[m], rtypes.Function):
            objcost += 2.5
        elif retic_tyinstance(trg.members[m], rtypes.Object):
            objcost += 1.6 * len(trg.members[m].members)
        else:  # Otherwise the type is Int. May need to refine
            objcost += 1.3
    return (10.9 + objcost) * UNIT_OF_COST

def countSrc(src,factor):
    oocost = 0
    for m in src.members:
        if retic_tyinstance(src.members[m], rtypes.Object):
            oocost += len(src.members[m].members)*factor+5
        else:
            oocost += factor
    return oocost

def objToObjCreate(src,trg):
    if len(trg.members) == 1:
        ttype = next(iter(trg.members.values()))
        if retic_tyinstance(ttype, rtypes.Dyn):
            factor = 0.45
        elif retic_tyinstance(ttype, rtypes.Int):
            factor = 0.5
            ...
            factor = 0.46
        oocost = countSrc(src,factor)
        return (oocost+8.8)*UNIT_OF_COST
    else:
        print("P*Cost estimation not implemented for casts\n {} => {}\n".format(src, trg))
        return 12.3 * UNIT_OF_COST

# Approximate the cost of creating a cast src => trg
def create_cost(src, trg):
    if src == trg:
        return 0
    if retic_tyinstance(trg, rtypes.Dyn):
        if retic_tyinstance(src, rtypes.Function):
            return 9.7*UNIT_OF_COST
        elif retic_tyinstance(src, rtypes.Object):
            if all( retic_tyinstance(src.members[m],rtypes.Dyn) for m in src.members):
                return 0.62 * UNIT_OF_COST
            else:
                # newly implemented for object creation cost
                return objToDynCreate(src,trg)
        elif retic_tyinstance(src, rtypes.Class):
            if all( retic_tyinstance(src.members[m],rtypes.Dyn) for m in src.members):
                return 0.62 * UNIT_OF_COST
            else:
                # Convert the Class to an Object type and compute the cost. This is
                # based on the observation that the cast operation for Class and Objects are
                # similar in guarded.py.
                # newly implemented for object creation cost
                return objToDynCreate(rtypes.Object(' ',src.members),trg)
                print ("G*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
                return 1
        elif retic_tyinstance(src,typing.List):
            return 19.5 * UNIT_OF_COST
        elif retic_tyinstance(src,typing.Tuple):
            return 11.5 * UNIT_OF_COST
        elif retic_tyinstance(src,typing.Dict):
            sizes = src.create_cast_size() - 1
            return (20.6 + 0.6 * sizes)  * UNIT_OF_COST

        elif any(retic_tyinstance(src, collection) for collection in [typing.Set]):
            print ("H*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 1
        else: return 0.43*UNIT_OF_COST
    elif retic_tyinstance(src, rtypes.Dyn):
        if retic_tyinstance(trg, rtypes.Function):
            return 9.6*UNIT_OF_COST
        elif retic_tyinstance(trg, rtypes.Object):
            if all( retic_tyinstance(trg.members[m],rtypes.Dyn) for m in trg.members):
                return 0.62 * UNIT_OF_COST
            else :
                # newly implemented for object creation estimation
                return dynToObjCreate(src,trg)
                print ("I*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
                return 1
        elif retic_tyinstance(trg, rtypes.Class):
            if all( retic_tyinstance(trg.members[m],rtypes.Dyn) for m in trg.members):
                return 0.62 * UNIT_OF_COST
            else:
                print ("I*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            ...
            return 17.3*UNIT_OF_COST
        elif retic_tyinstance(trg,typing.List):
            size = trg.create_cast_size()
            if size <= 2:
                return 30.2 * UNIT_OF_COST
            else:
                return 30.2 + (size-2) * 1.1
        elif retic_tyinstance(trg,typing.Dict):
            size = trg.create_cast_size()
            return (35.5 + size * 1.2) * UNIT_OF_COST
        elif any(retic_tyinstance(trg, collection) for collection in [typing.Set]):
            print ("J*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 25.3*UNIT_OF_COST
        else:
            return 0.557*UNIT_OF_COST
    elif retic_tyinstance(src, rtypes.Function) and retic_tyinstance(trg, rtypes.Function):
        if retic_tyinstance(src, typing.Object):
            print ("K*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 45.3*UNIT_OF_COST
        elif retic_tyinstance(trg, typing.Function):
            print ("L*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 35.3*UNIT_OF_COST
        else:
            return 11 * UNIT_OF_COST
    elif retic_tyinstance(src, typing.Class):
        if retic_tyinstance(trg, typing.Class):
            print ("M*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 25.3*UNIT_OF_COST
        elif retic_tyinstance(trg, typing.Function):
            print ("N*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
            return 25.3*UNIT_OF_COST
        else: raise ReticUnimplementedException(src, trg)
    elif retic_tyinstance(src, typing.Tuple) and retic_tyinstance(trg, typing.Tuple) :
        return 15.1 * UNIT_OF_COST
    elif retic_tyinstance(src, typing.List) and retic_tyinstance(trg, typing.List) :
        sizes = src.create_cast_size() - 1
        sizet = trg.create_cast_size() - 1

        return (40.6 + 2.5 * (sizes + sizet) - 3 * abs(sizes - sizet) )  * UNIT_OF_COST

    elif retic_tyinstance(src, typing.Dict) and retic_tyinstance(trg, typing.Dict) :
        sizes = src.values.create_cast_size() - 1
        sizet = trg.values.create_cast_size() - 1

        return (54.8 + 3 * (sizes + sizet) - 2.5 * abs(sizes - sizet) )  * UNIT_OF_COST

    elif any(retic_tyinstance(src, collection) and retic_tyinstance(trg, collection) \
                 for collection in [typing.Set]):
        #retic_assert(retic_has_type(val, trg), '%s at line %s' % (msg, line))
        print ("O*Cost estimation not implemented for casts\n {} => {}\n".format(src,trg))
        return 32.3*UNIT_OF_COST
    else:#  retic_subcompat(src, trg):
        return objToObjCreate(src,trg)
    # else:
        # raise ReticUnimplementedException(src, trg)
...
def cast(env, ctx, val, src, trg, msg, cast_function='retic_cast', misc=None):
    global functionCastCost,functionCastCostNliLocal
        ...
        if flags.SEMANTICS == 'MONO':
            ...
            if not tyinstance(trg, Dyn):
                ...
                if tyinstance(trg, Int):
                    ...
                    args += [ast.List(elts=[ast.Str(s=x) for x in trg.members], ctx=ast.Load())]
                    numMems = len(trg.members)
                    cc += numMems * 0.7*UNIT_OF_COST
                else:
                    logging.warn('Inserting cast at line %s: %s => %s' % (lineno, src, trg), 2)
                ...
                # Inserted for calculating the cast overhead for transient semantics
                functionCastCost += cc
                functionCastCostNliLocal += cc * getGlobalNli()
            ...
            noCC = getattr(misc,"_noCastCal", False)
            if not noCC:
                cc = total_cost(src,trg)
                functionCastCost += cc
                # print ('#Cost of the cast {} => {} for {} is {}'.format(src,trg, astor.to_source(val), cc))
                # print ('#Cost of the cast {} => {} is {} for {} -- {}'.format(src,trg, cc,val, astor.to_source(val)))

                functionCastCostNliLocal += cc * getGlobalNli()

                # logging.info ('#Cost of the cast {} => {} is {} repeated {}\n'.format(src,trg, cc,getGlobalNli()))

                if isinstance(val,ast.Name) and retic_tyinstance(src, rtypes.Dyn) and any(retic_tyinstance(trg, collection) for collection in [typing.Set, typing.List, typing.Tuple, typing.Dict, typing.Function]):

                    name = val.id
                    if name in funcPVInfoStack[-1].castedVariables :
                        print('@@@@@@ {} was and is casted again')

                        with open("unimplemented.txt","a+") as err:
                            err.write ("{} was casted and is casted again: {} => {}".format(astor.to_source(val),src,trg))

                    if name not in funcPVInfoStack[-1].parAssigned and name in funcPVInfoStack[-1].parIdxMap:
                        parid = funcPVInfoStack[-1].parIdxMap[name]
                        settings.parCastedTimesAll[funcPVInfoStack[-1].name][parid] += getGlobalNli()

                # print("The function call type {}, expression: {}".format(type(val),astor.to_source(val)))
                elif retic_tyinstance(src, rtypes.Dyn) and any(retic_tyinstance(trg, collection) for collection in [typing.Set, typing.List, typing.Tuple, typing.Dict, typing.Function]) and isinstance(val,ast.Call) and isinstance(val.func,ast.Name):
                    callee = val.func.id
                    if callee in settings.parCastedTimesAll:
                        __val = settings.parCastedTimesAll[callee].get('ReturnStatement',0)
                        settings.parCastedTimesAll[callee]['ReturnStatement'] = __val + getGlobalNli()
                    # print("-------The casted value is a function call named: ", val.func.id)
...
class Typechecker(Visitor):
    ...
    def visitModule(self, n, env, misc):
        global counterIni, counterPrint
...
"""
            ...
            initCIDict = ast.parse(initCIDictStr)
            initCIDict.lineno = -1

            counterIni.append(initCIDict)
...
"""
            ...
            dumpCIDict = ast.parse(dumpCIDictStr)
            dumpCIDict.lineno = -1

            counterPrint.append(dumpCIDict)
        return ast.Module(body=counterIni + body+ counterPrint)
...
## STATEMENTS ##
    ...
    def visitFunctionDef(self, n, env, misc): #TODO: check defaults, handle varargs and kwargs
        global functionCastCost, functionCastCostNliLocal
        global functionCosts, functionCostsNli
        global definedFuncs
        global counterDecls
        ...
        method_or_func_name = n.name if misc.cls is None else misc.cls.name + n.name + '__Class'
        if flags.BIT_STRING_GENERATION and not method_or_func_name in definedFuncs and method_or_func_name is not 'main':
            # settings.bit_string_info += 'Parameter and return types of {} are: {} -> {}\n'.format(n.name,froms,to)

            for (pn,ty) in froms.parameters + [('',to)]:
                if ty is Dyn:
                    settings.bit_string_info += '0,'
                else:
                    settings.bit_string_info += '1,'

        # Note, the following line should not be moved to a position that is too early
        definedFuncs.add(method_or_func_name)
        ...
        # print("Function {} has parameters: {}".format(n.name,argnames))
        ...
        valueCastedAsArg = {}
        idx = 0
        for fname in argnames:
            parIdxMap[fname] = idx
            parCastedTimes[idx] = 0
            valueCastedAsArg[idx] = 0
            parCastedTimes["ReturnStatement"] = 0
            idx += 1


        funcPVInfoStack.append(PerFunc(method_or_func_name,parIdxMap,set(),parAssigned) )
        settings.parCastedTimesAll[method_or_func_name] = parCastedTimes
        settings.valuesCastedWithinFunc[method_or_func_name] = valueCastedAsArg
        # print("---Function name stack: ",funcNameStack)

        ## for cast extraction ends
        ...
        logging.debug('Function %s typechecker starting in %s' % (method_or_func_name, misc.filename), flags.PROC)
        ...
        logging.debug('Function %s typechecker finished in %s' % (method_or_func_name, misc.filename), flags.PROC)
        ...
        # print("##Inside {}, the total cast cost is {}".format(n.name,functionCastCost))
        # print("##Inside {}, the total cast cost is {} with loops".format(n.name,functionCastCostNliLocal))
        functionCosts[method_or_func_name] = functionCastCost
        functionCostsNli[method_or_func_name] = functionCastCostNliLocal

        counters = counterDecls[:]
        counterDecls = []

        # print('??????',funcPVInfoStack)
        funcPVInfoStack.pop()

        # print("The return statement is: ", astor.to_source(body[-1]) if hasattr(n, 'returns') else "No returns")
        return [ast_trans.FunctionDef(name=name, args=args,
                                      body=counters+argchecks+body, decorator_list=decorator_list,
    ...
    def visitReturn(self, n, env, misc):
        if n.value:
            ...
            if ty != misc.ret and any(retic_tyinstance(ty, collection) and retic_tyinstance(misc.ret, collection) for collection in [typing.Set, typing.List, typing.Tuple, typing.Dict, typing.Function]):
                funcPVInfoStack[-1].castedVariables.add('ReturnStatement')

            # print("+++++++Inside the function {}, the return is: {}".format(funcPVInfoStack[-1].name, astor.to_source(n.value)))
        else:
            value = None
    ...
    def visitAssign(self, n, env, misc):
        ...
        if targets:
            ...
            if isinstance(targets[0],ast.Name):
                ...
                if vty != meet and any(retic_tyinstance(vty, collection) or retic_tyinstance(meet, collection) for collection in [typing.Set, typing.List, typing.Tuple, typing.Dict, typing.Function]):
                    funcPVInfoStack[-1].castedVariables.add(tar)

                if tar in funcPVInfoStack[-1].parIdxMap:
                    funcPVInfoStack[-1].parAssigned.add(tar)
    ...
    def visitFor(self, n, env, misc):
        global counterIndex, counterIni, counterPrint, counterDecls, localCIdx, loopNameStack
        ...
        if not flags.NUMBER_LOOP_ITERATIONS:
            ...
            initCIStr = "CIDict['fci{}']=0".format(counterIndex)

            # printStr = "print(fci{})".format(counterIndex)
            incCIStr = "CIDict['fci{}'] += 1".format(counterIndex)

            incCI = ast.parse(incCIStr)
            incCI.lineno = n.lineno

            initCI = ast.parse(initCIStr)
            initCI.lineno = n.lineno

            declCIStr = "CIDict['localci{}']=0".format(localCIdx)
            incLocalCIStr = "CIDict['localci{}'] += 1".format(localCIdx)

            declCI = ast.parse(declCIStr)
            declCI.lineno = n.lineno

            incLocalCI = ast.parse(incLocalCIStr)
            incLocalCI.lineno = n.lineno

            counterIni.append(initCI)
            counterDecls.append(declCI)

            counterIncs = [incCI,incLocalCI]

        loopNameStack.append('fci{}'.format(counterIndex))

        counterIndex += 1

        loopNameStack.append('localci{}'.format(localCIdx))
        ...
        return [ast.For(target=target, iter=cast(env, misc.cls, iter, ity, iter_ty,
                        ...
                        body= counterIncs + targcheck+body, orelse=orelse, lineno=n.lineno)]
    ...
    def visitWhile(self, n, env, misc):
        ...
        return [ast.While(test=test, body=counterIncs+body, orelse=orelse, lineno=n.lineno)]
    ...
    def visitClassDef(self, n, env, misc): #Keywords, kwargs, etc
        ...
        settings.contains_classes = True
        ...
        ## Commented out for removing class proxies
...
#        print("Dict creation detected at", misc.filename, n.lineno)
...
#        print("Dict comprehension detected at", misc.filename, lineno)
...
#            print("Dict comprehension detected at", misc.filename, lineno)
    ...
    def visitCall(self, n, env, misc):
        global callGraph, callGraphNli

        visitedCG = getattr(misc,"_noCastCal",False)
        if not visitedCG:
            caller = find_caller(n)
            if isinstance(n.func,ast.Name):
                # print(n.func.id)
                callGraph.setdefault(caller,[]).append(n.func.id)

                callGraphNli.setdefault(caller,[]).append((n.func.id,getGlobalNli()))

            elif isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name):
                callGraph.setdefault(caller,[]).append("{}.{}".format(n.func.value.id,n.func.attr))

                callGraphNli.setdefault(caller,[]).append(("{}.{}".format(n.func.value.id,n.func.attr),getGlobalNli()))
                # print("{}.{}".format(n.func.value.id,n.func.attr))

        # print_ancestor_class_names(n)
            ...
            if tyinstance(funty, Dyn):
                ...
                if argcasts != None:
                    ...
                    for (v, s), t in argcasts:
                        ...
                        if not visitedCG:
                            # if (isinstance(n.func,ast.Name)):
                                # print("{} has the type {}".format(n.func.id,ty))
                            # else:
                                # print("{} has the type {}".format(n.func,ty))

                            if s != t and any(retic_tyinstance(s, collection) or retic_tyinstance(t, collection) for collection in [typing.Set, typing.List, typing.Tuple, typing.Dict, typing.Function]) and isinstance(n.func,ast.Name):
                                fname = n.func.id
                                if fname in settings.parCastedTimesAll:
                                    settings.valuesCastedWithinFunc[fname][paridx] += getGlobalNli()*settings.parCastedTimesAll[fname][paridx]
    ...
    def visitIndex(self, n, env, extty, misc, lineno):
        ...
        if tyinstance(extty, List):
            ...
            ty = extty.values
#            print("Dict index detected at", misc.filename, lineno)
