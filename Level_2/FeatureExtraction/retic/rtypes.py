...
if __name__ == '__main__' or __name__ == 'rtypes':
    import flags
else:
    from . import flags
...
class Function(PyType, Structural):
    ...
    def create_cast_size(self):
        return froms.create_cast_size() + to.create_cast_size()
...
class List(PyType, Structural):
    ...
    def create_cast_size(self):
        return 1 + self.type.create_cast_size()
...
class Dict(PyType, Structural):
    ...
    def create_cast_size(self):
        return 1 + self.keys.create_cast_size() + self.values.create_cast_size()
...
class Tuple(PyType, Structural):
    ...
    def create_cast_size(self):
        return sum ([e.create_cast_size() for e in self.elements]) + 1
...
class Object(PyType, Structural):
    ...
    def create_cast_size(self):
        return sum ([self.members[m].create_cast_size() for m in self.members] ) + 1
...
class Class(PyType, Structural):
    ...
    def create_cast_size(self):
        return sum ([self.members[m].create_cast_size() for m in self.members] ) + \
               sum ([self.members[m].create_cast_size() for m in self.instance_members] ) + 1
...
class NamedParameters(ParameterSpec):
    ...
    def create_cast_size(self):
        return 1 + sum([t.create_cast_size() for k,t in self.parameters] )
...
class AnonymousParameters(ParameterSpec):
    ...
    def create_cast_size(self):
        return 1 + sum([t.create_cast_size() for t in self.parameters] )
