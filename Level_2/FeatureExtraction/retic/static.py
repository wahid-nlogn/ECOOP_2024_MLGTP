...
if __name__ == '__main__' or __name__ == 'static':
    import importer
    import gatherers
    import typing
...
def addTypesToEnv(env):
        env[Var("sorted")] = rtypes.Function(rtypes.DynParameters, rtypes.List(rtypes.Dyn))    
        # Commented out the following line for cpu-test benchmark, because
        # the source file contains calls such as int(value.lstrip('0x'), 16)
        # env[Var("int")]= rtypes.Function(rtypes.AnonymousParameters([rtypes.Dyn]), rtypes.Int)
        env[Var("len")]= rtypes.Function(rtypes.AnonymousParameters([rtypes.Dyn]), rtypes.Int)
        #env[Var("float")]= rtypes.Function(rtypes.AnonymousParameters([rtypes.Dyn]), rtypes.Float)
        env[Var("list")] = rtypes.Function(rtypes.DynParameters, rtypes.List(rtypes.Dyn))
        env[Var("range")] = rtypes.Function(rtypes.DynParameters, rtypes.Iterable(rtypes.Dyn))
        env[Var("xrange")] = rtypes.Function(rtypes.DynParameters, rtypes.Iterable(rtypes.Dyn))
